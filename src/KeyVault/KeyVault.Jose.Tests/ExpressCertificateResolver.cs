...
{
    internal class ExpressCertificateResolver : IKeyResolver
    {
        ...
        public ExpressCertificateResolver( KeyVaultClient.AuthenticationCallback authenticationCallback )
        ...
        public ExpressCertificateResolver( KeyVaultClient client )
        {
            if ( client == null )
        ...
        /// <param name="authenticationCallback">Key Vault authentication callback</param>
        public ExpressCertificateResolver( string vaultName, KeyVaultClient.AuthenticationCallback authenticationCallback )
        ...
        /// <param name="vaultName">The URL for the Key Vault, e.g. https://myvault.vault.azure.net/ </param>
        /// <param name="client">Key Vault client</param>
        public ExpressCertificateResolver( string vaultName, KeyVaultClient client )
        {
        ...
        private Task<IKey> ResolveKeyFromSecretAsync( string sid, CancellationToken token )
        {
            ...
            return _client.GetSecretAsync( sid, token )
                .ContinueWith<IKey>( task =>
                {
                    ...
                    if ( secret != null && string.Equals( secret.ContentType, "application/pkcs12", StringComparison.OrdinalIgnoreCase ) )
                    {
                        var certificateBytes = FromBase64UrlString( secret.Value );

                        if ( certificateBytes != null )
                        {
                            X509Certificate2Collection collection = new X509Certificate2Collection();

                            collection.Import( certificateBytes );
